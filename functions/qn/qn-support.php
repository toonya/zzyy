...
function _file_upload( $object , $file_path , $opt = array()){
	...
	//$key1 = "file_name1";
	$accessKey = get_option('OSS_ACCESS_ID');
	$secretKey = get_option('OSS_ACCESS_KEY');
	...
	$putExtra->Crc32 = 1;
	list($ret, $err) = Qiniu_PutFile($upToken, $object, $file_path, $putExtra);
...
function delete_remote_file($file)
{
	...
	$err = Qiniu_RS_Delete($client, $bucket, $file_path);
